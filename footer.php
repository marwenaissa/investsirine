<footer class="w-full px-4 py-8 bg-black text-white rounded-lg">
  <div class="container mx-auto flex flex-col md:<?= $lang === 'ar' ? 'flex-row-reverse' : 'flex-row' ?> gap-8">

    <!-- Bloc Contact -->
    <div class="flex-1">
      <h3 class="text-xl font-bold mb-4 <?= $lang === 'ar' ? 'text-right' : '' ?>"><?= $tr['contact_info'] ?></h3>
      <p class="mb-2 flex items-center <?= $lang === 'ar' ? 'flex-row-reverse text-right' : '' ?>">
    ...
    <div class="flex-1">
      ...
      <div class="flex items-center gap-4 text-white <?= $lang === 'ar' ? 'flex-row-reverse justify-start' : '' ?>">
        <!-- Instagram -->
